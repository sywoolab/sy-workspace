...
async function hydrateLiveSnapshot() {{
  try {{
    const qs = `ts=${{Date.now()}}`;
    const rawUrl = `https://raw.githubusercontent.com/sywoolab/sy-workspace/main/docs/ib_live.json?${{qs}}`;
    let r = await fetch(rawUrl, {{cache: 'no-store'}});
    if (!r.ok) r = await fetch(`ib_live.json?${{qs}}`, {{cache: 'no-store'}});
