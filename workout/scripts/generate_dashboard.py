...
def main():
    ...
    def _training_diagnosis(log, today_str):
        ...
        if bad_count >= 2:
            ...
            verdict = ('yellow', '부분 보완 필요', '자전거 기반은 좋음. 러닝·브릭 비중 높여야 대가야 A레이스 목표 달성 가능.')
        else:
...
"""
    ...
    all_races_ext = [
        ...
        ("2026-10-18", "대가야 올림픽", "10/18~19 개최 · 수영 1.5+자전거 40+러닝 10km", "🎯 A레이스 후보: sub-2:45, 연기일 확정 대기"),
        ("2026-10-25", "통영 월드컵", "10/24~25 개최 · 업다운 난코스 · 수영 1.5+자전거 40+러닝 10km", "신청완료 · 예비 A/C레이스: 수영 즐기기 + 내 페이스 완주, 대가야 불가 시 A 승격"),
    ]
